...
const buildFallbackMessage = (fallbackType, filters, categoryLabel) => {
	...
	if (fallbackType === 'budget') {
		return `Nu am găsit evenimente ${filters.budget === 'free' ? 'gratuite' : 'în buget'} în ${cityLabel}, dar aceste experiențe de ${categoryLabel} s-ar putea să te convingă să faci o excepție:`;
	}

	if (fallbackType === 'city') {
		return `E liniște în ${cityLabel} pe partea de ${categoryLabel}. Am găsit însă aceste evenimente tari în alte orașe, dacă plănuiești o ieșire:`;
	}

	if (fallbackType === 'none') {
		return 'Pauză de idei! 🕵️‍♂️ Nu am găsit nimic pe profilul tău acum. Ce-ar fi să încerci o altă categorie sau să cauți într-o perioadă mai aglomerată?';
	}
...
const getFallbackBadge = (fallbackType) => {
	if (fallbackType === 'budget') return { icon: '💸', label: 'Relaxare buget' };
	if (fallbackType === 'city') return { icon: '📍', label: 'Relaxare oraș' };
	return { icon: '🔍', label: 'Eșec total' };
};
...
const RecommendationResultsPage = () => {
	...
	const refreshResults = async () => {
		try {
			...
			setLiveFallbackMessage(nextMessage);
	...
	const fallbackBadgeLive = getFallbackBadge(liveFallbackType);
	...
	return (
		<div className="recommendation-page container-max">
			<header className="recommendation-header">
				...
				{liveFallbackMessage ? (
					<div className="recommendation-fallback-note">
						<span className="recommendation-fallback-badge-icon" aria-hidden="true">{fallbackBadgeLive.icon}</span>
						<div className="recommendation-fallback-copy">
							<strong>{fallbackBadgeLive.label}</strong>
							<p>{liveFallbackMessage}</p>
						</div>
					</div>
				) : null}
				<p>
