...
import { FiCheckCircle, FiClock, FiMail, FiMapPin, FiPhone, FiShield, FiSlash, FiUsers } from 'react-icons/fi';
import API from '../api';
...
const verificationPillClassMap = {
  unverified: 'neutral',
  pending: 'warning',
  verified: 'success',
  rejected: 'danger'
...
const AdminOrganizationsPage = ({ user, handleLogout }) => {
  ...
  return (
    <AdminShell user={user} handleLogout={handleLogout} title="Organizatori" subtitle={`${stats.total} organizatori înregistrați`}>
      ...
      <section className="admin-grid-4">
        <div className="admin-card compact">
          <div className="admin-stat"><div><label>TOTAL</label><strong>{stats.total}</strong></div><div className="admin-stat-icon blue"><FiUsers /></div></div>
        </div>
        <div className="admin-card compact">
          <div className="admin-stat"><div><label>ACTIVE</label><strong className="admin-text green">{stats.active}</strong></div><div className="admin-stat-icon green"><FiCheckCircle /></div></div>
        </div>
        <div className="admin-card compact">
          <div className="admin-stat"><div><label>SUSPENDATE</label><strong className="admin-text red">{stats.suspended}</strong></div><div className="admin-stat-icon red"><FiSlash /></div></div>
        </div>
        <div className="admin-card compact">
          <div className="admin-stat"><div><label>ÎN AȘTEPTARE</label><strong className="admin-text orange">{stats.pending}</strong></div><div className="admin-stat-icon orange"><FiClock /></div></div>
        </div>
      </section>
      ...
      <section className="admin-grid-4" style={{ gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}>
        {filteredOrganizations.map((organization) => {
          const emailParts = organization.owner?.email?.split('@');
          const maskedEmail = emailParts ? `${emailParts[0][0]}***@${emailParts[1]}` : '-';
          const revealed = revealedEmails.has(organization.id);
          const phoneRevealed = revealedPhones.has(organization.id);
          const maskedPhone = organization.officialPhone ? `${organization.officialPhone.slice(0, 3)}****${organization.officialPhone.slice(-2)}` : '-';
          ...
          return (
            <article key={organization.id} className="admin-card">
              <div className="admin-card-header">
                <div className="admin-row-inline">
                  ...
                  <div>
                    ...
                    <p className="admin-card-subtitle">{organization.owner?.name || 'Fără proprietar'}</p>
                  </div>
                </div>
              ...
              <div className="admin-row-stack">
                <div className="admin-row-inline">
                  <FiMail />
                  <button type="button" className="admin-email-reveal" onClick={() => toggleEmail(organization.id)} title={revealed ? 'Ascunde email' : 'Afișează email'}>
                    {revealed ? organization.owner?.email : maskedEmail}
                  </button>
                </div>
                ...
                <div className="admin-row-inline">
                  <FiPhone />
                  <button type="button" className="admin-email-reveal" onClick={() => togglePhone(organization.id)} title={phoneRevealed ? 'Ascunde telefon' : 'Afișează telefon'}>
                    {organization.officialPhone ? (phoneRevealed ? organization.officialPhone : maskedPhone) : 'Telefon lipsă'}
                  </button>
                </div>
              </div>
              ...
              <div className="admin-actions admin-section-spacing" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                {organization.adminStatus === 'active' ? (
                  <button type="button" className="admin-action-button danger" disabled={processingId === organization.id} onClick={() => toggleStatus(organization.id, 'suspended')}>Suspendă</button>
                ) : (
                  <button type="button" className="admin-action-button success" disabled={processingId === organization.id} onClick={() => toggleStatus(organization.id, 'active')}>Activează</button>
                )}
                <span className={`admin-pill ${verificationPillClassMap[organization.verificationStatus] || 'neutral'}`}>
                  {verificationLabelMap[organization.verificationStatus] || organization.verificationStatus}
                </span>
              </div>
            </article>
          );
